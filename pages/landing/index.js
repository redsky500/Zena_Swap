...
function Landing({ changeTheme }) {
  ...
  useEffect(() => {
    newCardUI = x.map((item, index) => {
      return <ZenaHalfBorder>cardUI(item.title, item.value)</ZenaHalfBorder>;
    });
  }, []);
    ...
    <div className={classes.ffContainer}>
      ...
      <div className={classes.contentContainerFull}>
        ...
        <div className={classes.cardWrapper}>
          {x.map((card) => {
            return <ZenaHalfBorder card={cardUI(card.title, card.value)} />;
          })}
        </div>
      </div>
      <div id="info" className={classes.contentContainerFullTwo}></div>
    </div>
  );
}
